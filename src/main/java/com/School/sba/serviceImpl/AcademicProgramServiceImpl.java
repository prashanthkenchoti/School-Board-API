...
public class AcademicProgramServiceImpl  implements AcademicProgramService{
	...
	private AcademicProgram mapToAcademicProgram(AcademicProgramRequestDTO academicProgramRequestDTO)
	{
		return AcademicProgram.builder()
				.programName(academicProgramRequestDTO.getProgramName())
				.beginsAt(academicProgramRequestDTO.getBeginsAt())
				.endsAt(academicProgramRequestDTO.getEndsAt())
				.build();
	}
	
	private AcademicProgramResponseDTO maptoAcademicProgramResponseDTO(AcademicProgram academicProgram)
	{
	...
	public ResponseEntity<ResponseStructure<AcademicProgramResponseDTO>> addAcademicProgram(
			...
			return new ResponseEntity<ResponseStructure<AcademicProgramResponseDTO>>(responseStructure,HttpStatus.CREATED);
	...
	public ResponseEntity<ResponseStructure<List<AcademicProgramResponseDTO>>> findAllAcademicPrograms(int schoolId) {
		...
		schoolRepository.findById(schoolId).map(k -> {
			...
			else
			{
				...
				return new ResponseEntity<ResponseStructure<List<AcademicProgramResponseDTO>>> (academicProgramResponseList,HttpStatus.FOUND);
